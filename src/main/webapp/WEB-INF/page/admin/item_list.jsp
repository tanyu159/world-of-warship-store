...
<html>
    ...
    <body class="side_menu_active side_menu_expanded">
        <div id="page_wrapper">
                        ...
                        <div class="col-md-12">
                        	...
                        	<form id="search_form" action="#${pageContext.request.contextPath}/admin/items" method="post">
	                            ...
	                            <div class="row">
	                                ...
	                                <!--按舰艇类型或者国籍国籍选择-->
	                                <div class="col-md-1">
	                                    <button style="width:80px" id="search_tagsAndPlatform_btn" type="button" data-toggle="modal" data-target="#tagsAndplatfromLayer"  class="btn btn-primary">国籍/类型</button>
	                                </div>

                                    <!--国籍id name要与数据库中字段名对应-->
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <table class="table table-yuk2 toggle-arrow-tiny" id="footable_demo" data-filter="#textFilter" data-page-size="10">
                                        ...
                                        <!--用foreach遍历实现获得到数据库中所有数据,itemList与controller那个对应-->
                                        
                                        <tbody id="item_info_date">
	                                        	<tr>
	                                                <td>${item.item_id}</td>
	                                                <td>${item.item_name}</td>
													<!--这两个属性可以为多个值，且用#号进行拆分开,实现映射后显示中文,方法实现见该jsp最下面的init函数-->
													<td class="item_tag_name">${item.item_tagids}</td>
													<td class="item_platform_name">${item.item_platform}</td>
	                                                <td>${item.item_discount=='0'||item.item_discount==null?'无':-item.item_discount}</td>
	                                                ...
	                                                <td>${item.is_upcoming==true?'即将上架':'否'}</td>
	                                                <td>${item.is_new==true?'新品':'否'}</td>
	                                                <td>${item.is_enable==true?'在架':'已下架'}</td>
	                                                <td>
	                                                	<a herf="#" id="edit_btn" class="btn btn-xs btn-info" data-toggle="modal" data-target="#editLayer" onclick="editItems(${item.item_id})">修改</a>
	                                                	<a herf="#" id="del_btn" class="btn btn-xs btn-danger" onclick="deleteItems(${item.item_id})">删除</a>
	                                                </td>
	                                           	</tr>
                                        </tbody>
										</c:forEach>
                ...
                <div class="modal-dialog modal-content">
                   ...
                   <div class="modal-body">
	                    ...
	                    <form class="form-horizontal" id="add_items_form" enctype="multipart/form-data">
							...
							<!-- 名称 -->
							<div class="form-group">
								<label for="add_item_name" class="col-sm-2 control-label">舰艇名称</label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="add_item_name" placeholder="舰艇名称" name="item_name">
								</div>
							</div>
							
							<!-- 国籍 -->
							<div class="form-group">
								<label for="add_item_tagids" class="col-sm-2 control-label">国籍</label>
								<div class="col-sm-10">
									<div>
                                        <!--隐藏域，name属性要和数据库中的字段名对应-->
							...
							<!-- 类型 -->
							<div class="form-group">
								<label for="add_item_platform" class="col-sm-2 control-label">类型</label>
								<div class="col-sm-10">
									<div>
                                        <!--隐藏域，name属性要和数据库中的字段名对应-->
							...
							<!-- 封面 -->
                            <div class="form-group">
								<label for="add_item_cap_image" class="col-sm-2 control-label">舰艇封面</label>
								<div class="col-sm-10">
            ...
            <div class="modal fade" id="editLayer">
                <div class="modal-dialog modal-content">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                            <h4 class="modal-title">修改舰艇信息</h4>
                        </div>
                   	  <div class="modal-body">
                          <input type="hidden" id="edit_is_hot" name="is_hot"/>
                          <input type="hidden" id="edit_is_hot_sale" name="is_hot_sale"/>
                          <input type="hidden" id="edit_is_specials" name="is_specials"/>
                          <input type="hidden" id="edit_is_upcoming" name="is_upcoming"/>
                          <input type="hidden" id="edit_is_new" name="is_new"/>
                          <input type="hidden" id="edit_is_enable" name="is_enable"/>
                        <!--描述：舰艇修改弹出层-->
	                    <%--<form class="form-horizontal" id="edit_items_form" enctype="multipart/form-data" method="post">--%>
							...
							<%--<!-- 名称 -->--%>
							<%--<div class="form-group">--%>
								<%--<label for="edit_item_name" class="col-sm-2 control-label">舰艇名称</label>--%>
								<%--<div class="col-sm-10">--%>
									<%--<input type="text" class="form-control" id="edit_item_name" placeholder="舰艇名称" name="item_name">--%>
								<%--</div>--%>
							<%--</div>--%>
							<%----%>
							<%--<!-- 国籍 -->	--%>
							<%--<div class="form-group">--%>
								<%--<label for="edit_item_tagids" class="col-sm-2 control-label">国籍</label>--%>
								<%--<div class="col-sm-10">--%>
                                    <%--<div>--%>
                                        <%--<!--隐藏域，name属性要和数据库中的字段名对应-->--%>
							...
							<%--<!-- 类型 -->	--%>
							<%--<div class="form-group">--%>
								<%--<label for="edit_item_platform" class="col-sm-2 control-label">类型</label>--%>
								<%--<div class="col-sm-10">--%>
                                    <%--<div>--%>
                                        <%--<!--隐藏域，name属性要和数据库中的字段名对应-->--%>
							...
							<%--<!-- 封面 -->	--%>
                            <%--<div class="form-group">--%>
								<%--<label for="edit_item_cap_image_src" class="col-sm-2 control-label">舰艇封面</label>--%>
								<%--<div class="col-sm-10">--%>
                            ...
                            <%--<div class="form-group">--%>
								...
								<%--<div class="col-sm-10">--%>
									<%--<input type="file" class="form-control" id="edit_item_cap_image" name="item_cap_image">--%>
								<%--</div>--%>
							<%--</div>--%>
							<%----%>
                            <%--<!-- 发售日期 -->	--%>
							...
							<%--</div>--%>
						<%--</form><!--描述：添加舰艇弹出层表单 end-->--%>
                          <form class="form-horizontal" id="edit_items_form" enctype="multipart/form-data">
                              <!-- item_id -->
                              <div class="form-group">
                                  <label for="edit_item_id" class="col-sm-2 control-label">id</label>
                                  <div class="col-sm-10">
                                      <input type="text" class="form-control" id="edit_item_id" placeholder="id" name="item_id" readonly="readonly">
                                  </div>
                              </div>
                              ...
                              <!-- 封面 -->
                              <div class="form-group">
                                  <label for="edit_item_cap_image" class="col-sm-2 control-label">舰艇封面</label>
                                  <div class="col-sm-10">
                              ...
                              <div class="form-group">
                                  ...
                                  <div class="col-sm-10">
                                      <div class="input-group date" id="dp_component_edit">
                                          <input type="text" class="form-control" placeholder="发售日期" name="item_release_date" id="edit_item_release_date">
                                          <span class="input-group-addon"><i class="icon_calendar" ></i></span>
                                      </div>
                                  </div>
                              </div>
            ...
            </div><!-- edit layer 舰艇修改弹出层 end-->
            
            <!-- search_tagsAndplatfrom_layer 选择国籍或类型的查询条件弹出层-->
            <div class="modal fade" id="tagsAndplatfromLayer">
                <div class="modal-dialog modal-content">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                            <h4 class="modal-title">选择国籍或者类型</h4>
                        </div>
                   	  <div class="modal-body">
                        <!--描述：选择国籍或类型的查询条件弹出层 表单-->
						...
						</form><!--描述：选择国籍或类型的查询条件弹出层 表单 end-->
                    </div>
                        <div class="modal-footer">
            ...
            </div><!-- search_tagsAndplatfrom_layer 选择国籍或类型的查询条件弹出层 end-->
                    ...
                    <ul>
                        <li class="first_level">
                            <a href="javascript:void(0)">
                                ...
                                <span class="menu-title">舰艇管理</span>
                            </a>
                            <ul>
                                <li class="submenu-title">舰艇管理</li>
                                <li><a href="#" data-toggle="modal" data-target="#addLayer">添加舰艇</a></li>
                                <li><a href="#">舰艇列表</a></li>
                            </ul>
                        </li>
                    </ul>
        ...
        $(function() {
			...
			function init() {
                ...
                //处理查询
                //拼接国籍id为字符串后发送给服务器,封装为一个方法来使用，到js中
                ...
                //实现添加弹窗标识位的checkbox点击后值得修改,选中为1，未选中为0 $(".xxx")这个叫做类查找器 $("#xxx")这叫id查找器
            ...
            //将查询弹出层中拼接好的id 设置到表单对应的隐藏域中
            function setTagidsAndPlatformToForm()
            {
                //前面是表单隐藏域，后面是查询弹出层隐藏域
			...
			function addItem() {
				$.ajax({
					...
					// data:$("#add_items_form").serialize(),serialize不支持文件上传
            ...
            //点击一条记录后面的修改按钮，打开修改弹出层，进行数据回显的查询
            ...
            function updateItem() {
                ...
                console.log($("#edit_items_form")[0]);
                // var formData = new FormData();
                // formData.append("testname","jianglian");
                $.ajax({
                    type:"POST",
